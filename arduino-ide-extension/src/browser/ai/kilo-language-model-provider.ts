...
async function kiloRequest(
  ...
  // 1. Try Node's native https module if available in Electron environment (bypasses browser CORS completely)
  try {
    const win = typeof window !== 'undefined' ? (window as any) : null;
    const https = win && typeof win.require === 'function' ? win.require('https') : null;
    if (https && typeof https.request === 'function') {
      return await new Promise((resolve, reject) => {
        const req = https.request('https://api.kilo.ai/api/gateway/chat/completions', {
          method: 'POST',
          headers: {
          ...
          res.on('end', () => {
            if (res.statusCode >= 400) {
              reject(new Error(`Kilo API error ${res.statusCode}: ${data}`));
              return;
            }
            try {
    ...
    // Fall back to fetch if https module is unavailable
  }
